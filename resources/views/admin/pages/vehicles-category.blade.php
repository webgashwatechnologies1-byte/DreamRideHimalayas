...
<body class="body header-fixed">
          ...
          <section class="profile-dashboard">
            ...
            <div class="inner-header mb-40 d-flex justify-content-between align-items-center">
              <div>
                ...
                <p class="des">Manage your listed Vehicles Category easily.</p>
              </div>
              <button id="addPlaceBtn" class="btn-main-small">+ Add Category</button>
            </div>
  ...
  <script>
    ...
    async function loadPlaces(page = 1, search = "") {
      ...
      try {
        const url = `${APP_URL}/api/v_cat?page=${page}&search=${encodeURIComponent(search)}`;
        ...
        if (Array.isArray(data)) {
          ...
          console.error('Unexpected API response shape for /api/v_cat:', data);
    ...
    function renderPlaces(v_cat = []) {
      const body = document.getElementById('placeTableBody');
      body.innerHTML = '';
  
      if (!v_cat.length) {
        body.innerHTML = `<tr><td colspan="3" class="text-center py-4 text-muted">No Vehicle Categories found.</td></tr>`;
        return;
      }
      ...
      v_cat.forEach(v_cat => {
        ...
        tr.innerHTML = `
          <td>${v_cat.id}</td>
          <td class="fw-semibold text-dark">${escapeHtml(v_cat.name)}</td>
          <td class="text-end">
            <div class="d-inline-flex gap-2">
              <button class="btn btn-sm btn-outline-primary btn-edit" data-id="${v_cat.id}" data-name="${escapeHtml(v_cat.name)}">Edit</button>
              <button class="btn btn-sm btn-outline-danger btn-delete" data-id="${v_cat.id}">Delete</button>
            </div>
          </td>
        `;
        body.appendChild(tr);
    ...
    async function editClickHandler(e) {
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`${APP_URL}/api/v_cat/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
            body: JSON.stringify({ name: result.value })
          });
          if (!res.ok) throw new Error('Update failed');
          toastMagic.success('Updated', 'Category updated successfully!');
          setTimeout(() => loadPlaces(currentPage, currentSearch), 400);

        } catch (err) {
          console.error(err);
          toastMagic.error('Error', 'Failed to update Category');
        }
      }
    }
    ...
    async function deleteClickHandler(e) {
      ...
      if (confirmed.isConfirmed) {
        try {
          const res = await fetch(`${APP_URL}/api/v_cat/${id}`, { method: 'DELETE' });
          if (!res.ok) throw new Error('Delete failed');
          toastMagic.success('Deleted', 'Category deleted');
          setTimeout(() => loadPlaces(currentPage, currentSearch), 400);
      ...
      if (addBtn) {
        addBtn.addEventListener('click', async () => {
          const { value: name } = await Swal.fire({
            ...
            showCancelButton: true,
            preConfirm: (val) => {
              if (!val || !val.trim()) Swal.showValidationMessage('Category name is required');
              return val && val.trim();
            }
          });
          ...
          if (name) {
            try {
              const res = await fetch(`${APP_URL}/api/v_cat`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name })
              });
              if (!res.ok) throw new Error('Add failed');
              toastMagic.success('Added', 'Category added successfully!');
              loadPlaces(currentPage,currentSearch);
            } catch (err) {
              console.error(err);
              toastMagic.error('Error', 'Failed to add Category');
            }
          }
        });
      }
