...
<body class="body header-fixed ">
          ...
          <section class="profile-dashboard">
            ...
            <div class="inner-header mb-40 d-flex justify-content-between align-items-center">
              <div>
                ...
                <p class="des">Manage your listed Tours and make updates easily.</p>
              </div>
              <button class="btn-main-small">
                <a href="{{ route('admin.tours.create') }}">+ Add Tour</a>
              </button>
            </div>
  ...
  <script>
    ...
    // 🔹 LOAD TOURS
    // =========================
    async function loadTours(page = 1, search = "") {
      ...
      tableBody.innerHTML = `
        <tr>
          <td colspan="3" class="text-center py-5">
            ...
            <div class="mt-2 text-muted">Loading tours...</div>
          </td>
        </tr>
      `;
      ...
      try {
        const response = await fetch(`${APP_URL}/api/tours?page=${page}&search=${encodeURIComponent(search)}`);
    ...
    function renderTours(tours = []) {
      ...
      if (!tours.length) {
        tableBody.innerHTML = `
          <tr>
            <td colspan="3" class="text-center py-4 text-muted">No tours found.</td>
          </tr>
        `;
        return;
      }
      ...
      tours.forEach(tour => {
        const imgUrl = tour.image
          ? `${APP_URL}/${tour.image.replace(/^\/+/, '')}`
          : `${APP_URL}/default-service.jpg`;
    
        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td><img src="${imgUrl}" alt="${escapeHtml(tour.name)}" class="img-fluid rounded" style="width:80px;height:60px;object-fit:cover;"></td>
          <td class="fw-semibold fs-5">${escapeHtml(tour.name)}</td>
          <td class="text-end fs-5">
            <div class="d-inline-flex gap-2">
              <button type="button" class="btn btn-light btn-sm border btn-edit" data-id="${tour.id}">
                <i class="bi bi-pencil me-1"></i> Edit
              </button>
              <button type="button" class="btn btn-danger btn-sm btn-delete" data-id="${tour.id}">
      ...
      document.querySelectorAll('.btn-edit').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          window.location.href = `${APP_URL}/admin/tours/edit/${id}`;
        });
      });
        ...
        btn.addEventListener('click', async () => {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              try {
                const resp = await fetch(`${APP_URL}/api/tours/${id}`, {
                  method: 'DELETE',
                  headers: { 'Content-Type': 'application/json' }
                });
                if (!resp.ok) throw new Error('Delete failed');
                toastMagic.success("Deleted!", "Tour deleted successfully.");
                loadTours(currentPage, currentSearch);
              } catch (err) {
                toastMagic.error("Error!", "Failed to delete tour.");
              }
            }
          });
        });
